...
export default function EditProfile() {
  ...
  const { apiData, isSuccess, isLoading, isError } = useApi(
    profile + name + "/media"
  );
  ...
  return (
    <section className="w-[95%] mx-auto mt-4">
      <button
        type="button"
        className="px-2 mt-2 mb-2 font-medium border rounded-lg hover:opacity-90 bg-error"
        onClick={() => dispatch(closeModal())}
      >
        X
      </button>
      <h1 className="w-11/12 mt-10 mb-12 text-4xl font-medium">
        Edit your profile
      </h1>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        {isSuccess && (
          <div className="success">Venue was successfully updated</div>
        )}
      </form>
    </section>
  );
