...
function UserBookings({ setActiveComponent, data }) {
  ...
  function DeleteButton({ id, setBookingsData }) {
    ...
    function handleDeleteBooking() {
      ...
      const updatedBookingsData = bookingsData.filter(
  ...
  return (
    <main className="pt-72 bg-[#FDFDFD] w-3.5/7 mx-auto xl:w-desktop mb-14">
      <Helmet>
        ...
        <meta
          name="description"
          content="Check out your bookings and delete bookings"
        />
      </Helmet>
      <ToastContainer />
      ...
      {bookingsData.length > 0 ? (
        <section className="flex flex-col gap-4">
          {bookingsData.map((bookings) => (
            <div
              key={bookings.id}
              className="flex items-center justify-between min-h-[10rem] gap-2 px-4 py-2 rounded-xl shadow-3xl group/item hover:cursor-pointer"
            >
              <div>
                <div className="flex justify-between">
                  <h2 className="mb-2 font-semibold sm:truncate">
                    {bookings.venue.name}
                  </h2>
                </div>
              ...
              <div className="flex flex-col gap-4">
                ...
                <Link
                  to={{
                    pathname: `/venue/${bookings.venue.id}`,
                    state: { from: location },
                  }}
